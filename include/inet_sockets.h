...
public:
    ...
    static int inet_listen(const char *service, int backlog);

    // init tcp/udp server socket, and bind port
    static int inet_bind(const char *service, int type);
...
private:
    static int inet_passive_socket(const char *service, int type, bool doListen, int backlog);
